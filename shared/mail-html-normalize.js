...
function normalizeOutgoingHtml(html, options) {
...
function prepareSignatureHtmlForSend(html) {
  let out = String(html || '').trim();
  if (!out) return out;
...
module.exports = {
  ...
  SIGNATURE_DEFAULT_LINK_COLOR,
  SIGNATURE_DEFAULT_TEXT_COLOR,
  decodeHtmlEntities,
  isEmptyHtmlBlock,
  normalizeOutgoingHtml,
  applySignatureLinkColors,
